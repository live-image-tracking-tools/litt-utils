...
def get_sorted_track_ids(graph: nx.DiGraph, tracklet_id_key: str = "tracklet_id") -> list[Any]:
    """Extract the lineage tree plot order of the tracklet_ids on the graph.

    Ensures that each tracklet_id is placed in between its daughter tracklet_ids
    and adjacent to its parent track id.
    ...
    while len(roots) > 0:
        ...
        for tracklet_id in roots:
            children = [
                tid for tid, ptid in tracklet_to_parent_tracklet.items() if ptid == tracklet_id
            ]
